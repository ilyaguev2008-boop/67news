...
# общая футбольная лента), Sky Sports и The Guardian — проверены лично
# 4 августа 2026 (перепроверены по запросу), отдают свежие новости на
# сегодняшний день. Это надёжнее фан-блогов, у которых RSS часто
# отваливается/меняется. Если какая-то лента всё же перестанет работать,
# fetcher.py просто пропустит её с предупреждением в консоли, не уронив
# весь бот.
FEEDS = [
...
# (og:image) и не берёт миниатюру из RSS-записи: у крупных изданий (BBC,
# Sky и т.п.) эти фото почти всегда содержат их логотип/вотермарку.
# Вместо этого бот ищет нейтральное фото по теме новости в интернете —
# сначала Wikimedia Commons (бесплатно, без ключа), затем Unsplash (если
# задан UNSPLASH_ACCESS_KEY). Поставь True, чтобы вернуть старое
# поведение (фото прямо со страницы статьи, быстрее, но с вотермарками).
...
# новости — гарантированно прикрепляется к посту, так как Telegram
# получает его как загруженный файл, а не по URL (частая причина сбоев
# "wrong type of the web page content" здесь исключена в принципе).
